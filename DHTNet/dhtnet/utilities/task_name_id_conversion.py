...
from dhtnet.paths import DHTNet_raw_data, preprocessing_output_dir, DHTNet_cropped_data, network_training_output_dir
...
def convert_id_to_task_name(task_id: int):
    ...
    if DHTNet_raw_data is not None:
        candidates_raw = subdirs(DHTNet_raw_data, prefix=startswith, join=False)
    else:
        candidates_raw = []

    if DHTNet_cropped_data is not None:
        candidates_cropped = subdirs(DHTNet_cropped_data, prefix=startswith, join=False)
    else:
        candidates_cropped = []
    ...
    if len(unique_candidates) > 1:
        raise RuntimeError("More than one task name found for task id %d. Please correct that. (I looked in the "
                           "following folders:\n%s\n%s\n%s" % (task_id, DHTNet_raw_data, preprocessing_output_dir,
                                                               DHTNet_cropped_data))
