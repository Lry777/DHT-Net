...
import numpy as np
from dhtnet.network_architecture import CNNBackbone
from dhtnet.network_architecture.neural_network import SegmentationNetwork
from dhtnet.network_architecture.DeTrans.DeformableTrans import DeformableTransformer
from dhtnet.network_architecture.DeTrans.position_encoding import build_position_encoding
...
class U_ResTran3D(nn.Module):
    def __init__(self, norm_cfg='BN', activation_cfg='ReLU', img_size=None, num_classes=None, weight_std=False):
        ...
        self.upsamplex2 = nn.Upsample(scale_factor=(2,2,2), mode='trilinear', align_corners=True)

        self.transposeconv_stage2 = nn.ConvTranspose3d(384, 384, kernel_size=(2,2,2), stride=(2,2,2), bias=False)
        self.transposeconv_stage1 = nn.ConvTranspose3d(384, 256, kernel_size=(2,2,2), stride=(2,2,2), bias=False)
        self.transposeconv_stage0 = nn.ConvTranspose3d(256, 64, kernel_size=(2,2,2), stride=(2,2,2), bias=False)

        self.stage2_de = ResBlock(384, 384, norm_cfg, activation_cfg, weight_std=weight_std)
        self.stage1_de = ResBlock(256, 256, norm_cfg, activation_cfg, weight_std=weight_std)
        self.stage0_de = ResBlock(64, 64, norm_cfg, activation_cfg, weight_std=weight_std)

        # self.ds2_cls_conv = nn.Conv3d(384, self.MODEL_NUM_CLASSES, kernel_size=1, stride=(2,1,1))
        # self.ds1_cls_conv = nn.Conv3d(192, self.MODEL_NUM_CLASSES, kernel_size=1, stride=(2,1,1))
        # self.ds0_cls_conv = nn.Conv3d(64, self.MODEL_NUM_CLASSES, kernel_size=1, stride=(2,1,1))

        self.ds2_cls_conv = nn.Conv3d(384, self.MODEL_NUM_CLASSES, kernel_size=1)
        self.ds1_cls_conv = nn.Conv3d(256, self.MODEL_NUM_CLASSES, kernel_size=1)
        ...
        self.encoder_Detrans = DeformableTransformer(d_model=384, dim_feedforward=1536, dropout=0.1, activation='gelu', num_feature_levels=2, nhead=8, num_encoder_layers=8, enc_n_points=4)
    ...
    def forward(self, inputs):
        ...
        # print(x_convs[0].shape)
        # print(x_convs[1].shape)
        # print(x_convs[2].shape)
        # print(x_convs[3].shape)
        ...
        # Multi-scale
        # print(x_trans.shape)
        # x = self.transposeconv_stage2(x_trans[:, 7680::].transpose(-1, -2).view(x_convs[-1].shape)) # SRGTHOR
        # skip2 = x_trans[:, 0:7680].transpose(-1, -2).view(x_convs[-2].shape)

        # x = self.transposeconv_stage2(x_trans[:, 6912::].transpose(-1, -2).view(x_convs[-1].shape))  # BCV
        # skip2 = x_trans[:, 0:6912].transpose(-1, -2).view(x_convs[-2].shape)
        
        x = self.transposeconv_stage2(x_trans[:, 4096::].transpose(-1, -2).view(x_convs[-1].shape))  # KITSM
...
class ResTranUnet(SegmentationNetwork):
    ...
    ResTran-3D Unet
    """
    def __init__(self, norm_cfg='IN', activation_cfg='ReLU', img_size=None, input_dim=None, num_classes=None, weight_std=False, deep_supervision=False):
        super().__init__()
        self.do_ds = False
