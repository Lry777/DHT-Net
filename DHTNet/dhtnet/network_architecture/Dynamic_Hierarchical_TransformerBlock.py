...
from dhtnet.network_architecture.Adaptive_Conv import DARConv3d
...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, in_chans=320, embed_dim=320,kernel_size=3,padding = 1,  is_pool = False,dropout = 0.):
        super().__init__()
        if is_pool:
            # self.proj = nn.Conv3d(in_chans, embed_dim, kernel_size=2, stride=2)
            # self.proj = DWConv3d_IN(in_ch=in_chans, out_ch= embed_dim,kernel_size=3, stride=2)
            self.proj = nn.Sequential(
                                      nn.MaxPool3d(2),
                                      # nn.Conv3d(in_chans, embed_dim, kernel_size=3, stride=1, padding=1),
                                      # DWConv3d_IN(in_ch=in_chans, out_ch= embed_dim,kernel_size=3, stride=1),
            ...
            # self.proj = DWConv3d_IN(in_ch=in_chans, out_ch= embed_dim,kernel_size=3, stride=1)
...
class DHTransformer(nn.Module):
    def __init__(self, in_chans=1, out_chans=32, paths = 3,
                 num_heads=8, mlp_ratios=2, is_pool = True, qkv_bias=False, qk_scale=None, drop_path_rate=0.,
