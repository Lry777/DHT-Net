...
from dhtnet.training.dataloading.dataset_loading import load_dataset, DataLoader3D, DataLoader2D, unpack_dataset
from dhtnet.training.loss_functions.dice_loss import DC_and_CE_loss
from dhtnet.training.network_training.network_trainer import NetworkTrainer
...
class nnUNetTrainer(NetworkTrainer):
    ...
    def process_plans(self, plans):
        ...
        feature_map = []
        feature = self.patch_size
        feature_map.append(tuple(feature))
        for i, pool_k in enumerate(pool_op_kernel_sizes):
            feature = np.divide(feature,pool_k).astype(int)
            feature_map.append(tuple(feature))
        save_pickle(feature_map, os.path.join(network_training_output_dir_base, 'feature_map.pkl'))
        # print(load_pickle(os.path.join(network_training_output_dir_base, 'feature_map.pkl')))
    ...
    def preprocess_patient(self, input_files):
        ...
        preprocessor_class = recursive_find_python_class([join(dhtnet.__path__[0], "preprocessing")],
                                                         preprocessor_name,
                                                         current_module="dhtnet.preprocessing")
        assert preprocessor_class is not None, "Could not find preprocessor %s in dhtnet.preprocessing" % \
                                               preprocessor_name
    ...
    def validate(self, do_mirroring: bool = True, use_sliding_window: bool = True, step_size: float = 0.5,
        ...
        if run_postprocessing_on_folds:
            # in the old dhtnet we would stop here. Now we add a postprocessing. This postprocessing can remove everything
